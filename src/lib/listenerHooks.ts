 ...
 *
 * 【这个文件是干啥的？】
 *    想象你参加一个国际会议，有人在讲外语，你戴着同声传译的耳机。
 *    发言人每说一句话，耳机里就传来翻译后的中文。
 *
 *    这个文件就是那个"同声传译员"：
 *    - RTC SDK（实时通信SDK）产生的各种事件，就像是"发言人"说的各种话
 *    - 我们的 Hook（翻译员）负责把这些事件翻译成 Redux 能理解的"指令"
 *    - Redux（会议室管理员）收到指令后，更新会议室的状态
 *    - UI（投影屏幕）根据最新状态，自动刷新显示
 *
